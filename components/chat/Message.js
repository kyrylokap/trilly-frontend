...
function Message({message, setInput,  setChangingMessage,  setId, setIsChanging, selectedChatId, stompClient}){
    ...
    return(
        <div onClick={() => {
            ...
            }}}  className={`flex flex-col ${localStorage.getItem("username") === message.sender ? 'items-end' : 'items-start'} relative  inline-block`}>

            {messageMenu && <MessageMenu selectedChatId={selectedChatId}  setIsChanging={setIsChanging} stompClient={stompClient}
                            setId={setId} id={message.id} setChangingMessage={setChangingMessage} message={message} messageType={message.type}/>}
